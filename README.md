...
抽出した銘柄について、前日比・前週比・前月比を計算し、銘柄別チャートとともにレポートを生成します。生成した加工済みデータとPDFレポートは Cloudflare R2 にアップロードし、レポートURLを LINE Messaging API であなたのLINEに毎日通知します。
